  ...
  return (
    <main className="max-w-lg mx-auto p-6">
      <p className="text-[var(--text-muted)] text-sm mb-2">
        <Link href="/admin" className="transition-smooth hover:text-white">← イベント一覧</Link>
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <label className="block text-sm text-[var(--text-muted)]">イベント名</label>
        <input
          type="text"
          value={name}
        ...
        <div className="flex gap-3 pt-4">
          <button
            type="button"
            onClick={() => router.back()}
            className="rounded-2xl border border-[var(--border-light)] px-4 py-2 font-medium transition-smooth hover:bg-[var(--surface-elevated)] active:scale-[0.98] focus-ring"
          >
            キャンセル
          </button>
          <button
            type="submit"
            disabled={loading}
            className="rounded-2xl bg-white px-6 py-2 font-semibold text-black transition-smooth hover:bg-zinc-200 disabled:opacity-50 active:scale-[0.98] focus-ring"
          >
            {loading ? "作成中…" : "作成"}
          </button>
        </div>
